...
- 現行コードとの食い違い: 要望は 8b8a77f で、行の間の高さ1の線と、表示設定の文字とコントロールを小さくする形で実装された。縮小は 1cd6081 で、線はタブ機能を加えた 8e5e8cd で失われた（いずれも公開版 v1.0.3 に含まれる）。現行コードには行の間の線も表示設定を縮める指定もなく、あるのはタブの中身全体を囲む枠線だけである（横レイアウトでは、表示設定は行ではなく左側の列にある）。後続の担当: 未定（扱いは利用者の判断待ち）
