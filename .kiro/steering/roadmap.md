...
- [ ] ntfs-mft-scan -- ローカルの NTFS ドライブを管理者として走査するとき、目録（MFT）をまとめて読む走査方式を加え、**WizTree の最速モードと同等以上**を目指す。2026-09-25 の実測: この方式は**管理者が必須**（`FSCTL_QUERY_FILE_LAYOUT`）。WizTree はローカルのドライブ（191万ファイル）を**管理者で 3.67 秒**、このアプリは管理者なしの通常の走査で約7.1秒。使えない条件（NAS・NTFS 以外・管理者でない）では通常の走査に戻る。Dependencies: scan-performance
- [ ] scan-enumeration-speed -- 通常の走査（管理者を必要としない経路）の速さを、管理者なしの WizTree と比べて詰める。**管理者なしの WizTree の値が未測定**のため、まずその計測から。差が小さければ着手しない判断もありうる。Dependencies: scan-performance, ntfs-mft-scan
