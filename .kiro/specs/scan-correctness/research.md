...
## 最終確認（validate-impl、2026-09-22）
- 判定: GO（1回目は NO-GO。走査の取り消し・中断の状態表示が上書きされ要件5.6 を満たさない問題を直してから GO）
- 機械的な確認: 警告なしのビルド、dotnet test 8/8、selfcheck 142/142、期待値データとの比較2種（物理サイズ換算なし・あり）一致、翻訳の網羅（言語13・キー82）問題なし、2形態の発行と起動確認成功
- 要件の網羅: 26/26。1.3、1.6、3.2、4.2、5.2、5.6、6.2 は自動の試験が無く、コードの読みと利用者による確認の手順で担保する
- 境界: 検証ツールの既存の判定規則（Compare、DetectSkippedPaths、MeasureClusterSize）と本スキャンの集計の処理に変更なし
