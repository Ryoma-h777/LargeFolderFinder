...
```
- 自動の既定値: ネットワークは 16、ローカルは論理プロセッサ数を 4〜16 に丸めた値。**初期値であり、ベンチマークの結果で確定し、確定した値と根拠を measurements.md に記録する**
...
- 1回ごとに1行（タブ区切り）: ラベル、本体の版、並列度（実際のワーカー数）、同時の列挙の最大、バッファ、所要時間（ミリ秒）、フォルダ数、ファイル数、総バイト数、スキップ数、最大の作業セット（MB）、走査後の管理ヒープ（MB）、要約値
