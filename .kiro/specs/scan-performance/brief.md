...
- **通常の列挙で WizTree と同等以上の速さ**（2026-09-22 利用者の決定）。対象は NAS と、管理者でない・NTFS 以外のローカル。WizTree もこれらの場面では MFT を読めず通常の列挙に戻るため、並列化の作り込みで上回る余地がある。ローカルの NTFS を管理者として走査する場面の目標は `ntfs-mft-scan` が持つ
...
- MFT（NTFS の全ファイルの目録）を直接読む走査方式（`ntfs-mft-scan` が担う。本スペックは通常の列挙の経路だけを速くする）
- インデックスの事前構築や常駐監視は本プロジェクトの範囲外
...
- **WizTree との比較を計測の手順に含める。** 同じ対象（NAS の共有、管理者でない状態のローカル）を同じ条件（初回か2回目以降か、ファイルキャッシュの状態）で両方走査し、所要時間を並べて記録する。WizTree は利用者の PC に入れて測り、同梱しない
