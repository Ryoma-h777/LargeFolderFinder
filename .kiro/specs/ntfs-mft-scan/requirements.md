...
3. **【2026-09-25 の決定・変更】管理者の権限を持つ利用者は、アプリの起動のときに昇格する**（マニフェストの `highestAvailable`）。管理者の人は起動のたびに権限の確認（UAC）を通り、以後は目録を読む速い方式が常に使える。**走査のたびに尋ねる処理は持たない**
4. **管理者の権限を持たない利用者は、そのまま通常の権限で起動できる**（`requireAdministrator` は採らない。起動できなくなり、将来のドラッグ＆ドロップも使えなくなるため）。その場合は通常の走査になり、昇格を促す表示もしない
5. 比較のために測った WizTree は、起動のたびに管理者を求める作り（管理者でないと起動できない）だった。本アプリはそこまで強くせず、管理者を持つ人だけ自動で昇格する形にする
...
- **In scope**: MFT を直接読む走査方式、使える条件の判定と通常の走査への切り替え、起動時の昇格（マニフェスト）、対象がフォルダのときの配下の絞り込み、走査の方式と結果の意味の利用者への提示（最小限）、MFT の走査の集計値を確かめる仕組み、WizTree の最速モードとの比較の計測と記録
...
### Requirement 3: 起動時の昇格

**Objective:** As a 利用者, I want 速い方式が使える環境では何もしなくても使えること, so that 走査のたびに操作を求められない

#### Acceptance Criteria

1. When 管理者の権限を持つ利用者がアプリを起動したとき, the Large Folder Finder shall 権限の確認（UAC）を経て管理者として起動する
2. If 起動のときの権限の確認が拒否されたとき, then the Large Folder Finder shall 起動を中止する（Windows の既定の動き）
3. The Large Folder Finder shall 管理者の権限を持たない利用者のアカウントでも、通常の権限で起動できる
4. Where アプリが管理者として動いていないとき, the Large Folder Finder shall 走査のたびに昇格を促さず、通常の走査で走査する
5. The Large Folder Finder shall 走査の開始のために利用者へ追加の操作を求めない
