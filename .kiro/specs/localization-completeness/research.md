...
- **Implications**: 重複は「読み込み不能」ではなく別の問題として、YAML の表現モデル（キーの並び）から検出する必要がある。要件3.5を事実に合わせて改め、3.10 を追加した
...
### Decision: 重複キーは YAML の表現モデルで検出する
- **Context**: 要件3.10。辞書への読み込みでは重複が黙って上書きされる
- **Selected Approach**: `YamlStream` で文書を読み、最上位のマッピングのキーの出現回数を数える。辞書への読み込み（読み込み可否の判定）とは別に行う
