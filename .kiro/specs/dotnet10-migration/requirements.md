...
本フィーチャーは、Large Folder Finder の実行基盤を .NET 10 へ移し、後続のすべてのスペックが前提とする土台（長いパスを扱える基盤、テストの基盤、自動ビルド）を整えるものである。

利用者から見た機能と操作は変えない。変わるのは、配布物の形（自己完結版とフレームワーク依存版の2つ）と、260文字を超える階層が走査結果に現れるようになることだけである。

## Boundary Context

- **In scope**: 対象フレームワークの変更と、それに伴うコードとプロジェクト設定の修正、Costura.Fody と Fody の除去と単一ファイル発行への置き換え、Ookii.Dialogs.Wpf の削除と標準のフォルダ選択ダイアログへの置き換え、依存の更新とバージョン下限の確定、SDK の固定、GitHub Actions による2形態の自動ビルド・起動確認・下書きリリース、テストプロジェクトの新設と期待値データとの突き合わせ、検証ツール（走査結果の検証、翻訳の網羅の検証）の移行への追従、`ThirdPartyNotices.txt` と README の更新、走査結果の差の確認と期待値データの更新
...
- **Adjacent expectations**: `scan-golden-baseline` の期待値データと比較ツールを、移行の影響を測る基準として使う。`localization-completeness` が加える翻訳の網羅の検証ツールも、移行後に動き続ける必要がある。後続のすべてのスペックは、本スペックが整えるテスト基盤と CI の上で作業する
...
### Requirement 5: 自動ビルドと配布物の生成

**Objective:** As a 開発者, I want タグを付けるだけで2形態の配布物が用意されること, so that リリースの手間が移行前より増えない

#### Acceptance Criteria

1. When 開発者がバージョンのタグを送ったとき, the 自動ビルド shall 2形態の配布物をビルドし、それぞれを配布用の書庫にまとめる
2. When 2形態の配布物ができたとき, the 自動ビルド shall それぞれの実行ファイルが実際に起動して使える状態になることを確かめ、起動に失敗したときは処理を失敗として報告する
3. When ビルドと起動の確認が成功したとき, the 自動ビルド shall 2つの書庫を添えた公開前の下書きのリリースを作成する
4. The 自動ビルド shall リリースの公開そのものは行わず、人の操作に委ねる
5. When 開発者が変更を送ったとき（タグを伴わない場合を含む）, the 自動ビルド shall ビルドとテストを実行し、失敗を報告する
6. The 自動ビルド shall ビルドに使う開発キットの版を固定し、固定した版を記録に残す
7. If 固定した版で配布物の起動に問題が見つかったとき, then the 開発者 shall 版を変えて確かめ、選んだ版とその理由を記録する
8. The 自動ビルド shall 開発者の手元でも同じ手順で配布物を作れるようにする
...
3. When 自動ビルドが動いたとき, the 自動ビルド shall テストを実行し、失敗したときはビルドを失敗として扱う
...
3. The 翻訳の網羅の検証ツール shall 移行後も、これまでと同じコマンドと終了コードで動作し、自動ビルドの中で実行されて、問題があるときは自動ビルドを失敗として扱わせる
