...
{
    /// <summary>ディレクトリの文字数境界（248文字）またはファイルパスの文字数境界（260文字）を超える（要件3.1）。</summary>
    LongPath,
...
public sealed class FixtureSpec
{
    ...
    internal static FixtureSpec CreateStandard()
    {
        ...
        // --- 長いパス（ASCII）---
        // ディレクトリの文字数境界（248文字）とファイルパスの文字数境界（260文字）は異なるため、
        // 両方をまたぐ単一の階層として構成する（research.md: 日本語を含む長いパスの扱い）。
        // 数え方は文字数（WCHAR）。1階層50文字 × 5階層 + 区切り4文字 = 254文字で248文字境界を越える。
        AddNestedChain(
            items,
        ...
        // --- 長いパス（日本語）---
        // 文字数境界は日本語でもASCIIでも同じ248/260文字で発生する
